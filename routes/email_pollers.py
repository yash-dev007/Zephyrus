                        ...
                        if jm:
                            ...
                            if urgency in ("critical", "high"):
                                try:
                                    ...
                                    # Deep-link to open the original email in Zephyrus (if public URL is configured).
                                    ...
                                    # HTML alternative with a clickable "Open in Zephyrus" button
                                    import html as _h
...
def _scheduled_poll_once() -> dict:
    ...
    try:
        ...
        kind_expr = "zephyrus_kind" if "zephyrus_kind" in cols else "'scheduled' AS zephyrus_kind"
        ...
        for r in rows:
            ...
            try:
                ...
                # externally cron-driven `zephyrus-mail poll-scheduled`, or
                # an admin running the CLI manually alongside the in-process
                # one despite the ZEPHYRUS_INPROCESS_POLLERS=0 guidance) -
                ...
                zephyrus_kind = r[10] if len(r) > 10 else "scheduled"
                ...
                outer["X-Zephyrus-Origin"] = "zephyrus-ui"
                outer["X-Zephyrus-Kind"] = re.sub(r"[^A-Za-z0-9_.-]", "-", zephyrus_kind or "scheduled")[:64]
                outer["X-Zephyrus-Ref"] = sid
                if r[6]:
...
async def _scheduled_email_poller():
    ...
    also exposed via the `zephyrus-mail poll-scheduled` CLI for
...
def _inprocess_pollers_enabled() -> bool:
    """Honour `ZEPHYRUS_INPROCESS_POLLERS` — set to `0`/`false`/`no`/`off`
    to disable the asyncio tasks so a cron / systemd-timer setup driving
    `zephyrus-mail poll-scheduled` is the sole external driver. The legacy
    ...
    import os
    raw = os.environ.get("ZEPHYRUS_INPROCESS_POLLERS", "1").strip().lower()
...
def _start_poller():
    ...
    Skipped entirely when `ZEPHYRUS_INPROCESS_POLLERS=0` — use that when
    ...
    if not _inprocess_pollers_enabled():
        logger.info(
            "In-process email pollers disabled (ZEPHYRUS_INPROCESS_POLLERS=0); "
            "drive `zephyrus-mail poll-scheduled` externally."
        )
        return
    import asyncio
