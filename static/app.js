// ============================================
// Zephyrus UI — Main Application Orchestrator
...
function _isForegroundChatBusy() {
  ...
  return !!window.__zephyrusChatBusy
...
function _submitMobileQueuedInput(input) {
  ...
  window.__zephyrusQueueStreamingSubmit = now;
...
async function _refreshDefaultChat() {
  try {
    ...
    if (d && d.endpoint_url && d.model) {
      _defaultChat = d;
      try { window.__zephyrusDefaultChat = d; } catch (_) {}
      return d;
    }
  } catch (_) {}
  return null;
...
//  document.getElementById('chat-form').addEventListener('submit', chatModule.handleChatSubmit);
  ...
  if (exportPdfBtn) {
    exportPdfBtn.addEventListener('click', (e) => {
      ...
      const sessionName = meta ? meta.name : 'Zephyrus Chat';
  ...
  if (_opener) window._zephyrusRouteOpener = _opener;
  ...
  if (modelSortBtn && modelSortDropdown) {
    ...
    modelSortDropdown.querySelectorAll('.sort-option').forEach(opt => {
      opt.addEventListener('click', () => {
        const mode = opt.dataset.sort;
        Storage.set('zephyrus-model-sort', mode);
...
// Voice recording is handled by the dual-purpose send/mic button (see below)
  ...
  (function initModeToggle() {
    ...
    window.__zephyrusSetChatMode = setMode;
  ...
  const SPLASH_COUNT_KEY = 'zephyrus-tool-splash-counts';
  const SPLASH_MAX = 2;
  const _toolSplashes = {
  ...
  (function initModelPickerResponsive() {
    ...
    function checkPickerOverflow() {
      ...
      if (textarea) {
        textarea.setAttribute('placeholder', w < PLACEHOLDER_COMPACT_WIDTH ? 'Message...' : 'Message Zephyrus...');
      }
  ...
  if (incognitoBtn) {
    ...
    incognitoBtn.addEventListener('click', () => {
      ...
      if (chk.checked) {
        ...
        if (typeof window.__zephyrusSetChatMode === 'function') {
          window.__zephyrusSetChatMode('chat');
        } else {
        ...
        if (typeof window.__zephyrusSetChatMode === 'function') {
          window.__zephyrusSetChatMode(_restoreMode === 'chat' ? 'chat' : 'agent');
        }
  ...
  const UI_VIS_KEY = 'zephyrus-ui-visibility';
  ...
  (function migrateOldToolbarVis() {
    const OLD_KEY = 'zephyrus-toolbar-visibility';
    try {
  ...
  if (textarea) {
    ...
    window.addEventListener('zephyrus:chat-busy-change', () => _syncMobileEnterKeyHint(textarea));
      ...
      if (_shouldQueueFromMobileEnter(e, textarea) || (e.key === 'Enter' && !e.shiftKey && !e.isComposing && !isMobile)) {
        ...
        if (form) {
          if (_isForegroundChatBusy() && textarea.value && textarea.value.trim()) {
            ...
            window.__zephyrusQueueStreamingSubmit = Date.now();
          }
          _submitChatFormDirect(form);
        }
      }
...
// ============================================
function startZephyrusApp() {
  if (window.__zephyrusAppStarted) return;
  window.__zephyrusAppStarted = true;
  const _bumpChatPriority = (ms = 10000) => {
    try {
      window.__zephyrusChatBusyUntil = Math.max(window.__zephyrusChatBusyUntil || 0, Date.now() + ms);
    } catch (_) {}
  };
  ...
  if (documentModule) {
    ...
    if (_curSession && localStorage.getItem('zephyrus-doc-open-' + _curSession) === '1') {
  ...
  window._zephyrusBtnIcons = { send: _sendIcon, mic: _micIcon, stop: _stopIcon, newChat: _newChatIcon };
  ...
  if (sendBtn) {
    sendBtn.addEventListener('click', (e) => {
      ...
      if (sendBtn.dataset.mode === 'streaming') {
        if (hasText) window.__zephyrusQueueStreamingSubmit = Date.now();
        handleSubmit(e);
        return;
      }
  ...
  if (sessionModule) {
    ...
    sessionModule.loadSessions()
      ...
      .finally(() => {
        ...
        if (window._zephyrusRouteOpener) {
          try { window._zephyrusRouteOpener(); } catch (_) {}
          window._zephyrusRouteOpener = null;
        }
      });
  } else {
...
if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', startZephyrusApp, { once: true });
} else {
  startZephyrusApp();
}
