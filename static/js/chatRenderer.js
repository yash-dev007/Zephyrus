...
const _ACTION_RECENTS_KEY = 'zephyrus-msg-actions-recent';
 ...
 */
const _USER_ACTION_RECENTS_KEY = 'zephyrus-user-actions-recent';
    ...
    ctxRing.addEventListener('click', (e) => {
      ...
      if (compactBtn) {
        compactBtn.addEventListener('click', async (e) => {
          ...
          compactRole.textContent = 'Zephyrus';
...
export function addMessage(role, content, modelName, metadata) {
  try {
    ...
    var _roleText = role === 'user' ? 'You' : (isSlash || isCompacted) ? 'Zephyrus' : modelRouteLabel(replyModels.requestedModel, resolvedModel);
